            ...
            <table style={{ borderCollapse: 'collapse', width: '100%', tableLayout: 'fixed' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...thBase, width: 100 }}>MÉT</th>
                      ...
                      {isNota ? (
                        <>
                          ...
                          <td style={{ ...tdBase, textAlign: 'center', fontSize: 10, fontWeight: 800, color: NAVY, background: '#e7eefb' }}>
                            {row.metodo === 'Efectivo' ? 'EF' : row.metodo === 'Banco JORGE' ? 'BJ' : row.metodo === 'Terminal' ? 'TN' : row.metodo === 'Transferencia' ? 'TR' : 'BD'}
                          </td>
                          {/* Sin botón borrar */}
                          <td style={tdBase} />
                        </>
                        ...
                        <>
                          ...
                          {/* Método */}
                          <td style={{ ...tdBase, padding:0, width:100 }}>
                            <div style={{ display:'flex', height:'100%' }}>
                              {[['Efectivo','EF'],['Terminal','TN'],['Transferencia','TR'],['Banco JORGE','BJ']].map(([m, label], mi) => (
                                <div key={m} onClick={() => toggleMetodo(i, m)}
                                  style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight: row.metodo===m ? 800 : 600, color: row.metodo===m ? NAVY : '#bbb', cursor:'pointer', userSelect:'none', borderRight: mi < 3 ? `1px solid ${LINE_SOFT}` : 'none', background: row.metodo===m ? '#e7eefb' : 'transparent', transition:'background 0.1s' }}>
                                  {label}
                                </div>
                              ))}
                            </div>
